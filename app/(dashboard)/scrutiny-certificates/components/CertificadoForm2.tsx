...
export default function CertificadoForm({
  ...
  const [loadingCertificado, setLoadingCertificado] = useState(modo === "editar");
        ...
        try {
          ...
          if (escuelaId) {
            try {
              ...
              onEscuelaSeleccionada?.(res.data); // ← Esto actualiza el estado en MesaSelector
            } catch (err) {
              console.error("Error al cargar la escuela seleccionada:", err);
            }
          }
        } catch (error) {
