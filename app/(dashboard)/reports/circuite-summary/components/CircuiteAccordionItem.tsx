...
export function CircuiteAccordionItem({
  ...
  const chartData = buildChartData(circuite.resultados);
  const ordenCategorias = ["DIPUTADOS", "SENADORES"];
  
  return (
    <AccordionItem key={circuite.circuitoId} value={`establecimiento-${circuite.circuitoId}`} >
          ...
          <p>
            <span className="flex text-muted-foreground ">
              ...
              <span className="flex items-center text-xs">
                ...
                {circuite.resumen
                  ? `Votantes: ${circuite.resumen.electoresVotaron} - Sobres: ${circuite.resumen.sobresEnUrna}`
                  : "Sin datos"}
              </span>
            </span>
          </p>
        ...
        <Card className="mt-2 border-none">
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-[70%_30%] gap-6">
              ...
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData}>
                    ...
                    {getCategoriasUnicas(circuite.resultados).map((cat) => (
                      <Bar
                        key={cat}
                        dataKey={cat}
              ...
              <MesaRanking resultados={circuite.resultados} />
            </div>
            ...
            {mostrarVotosEspeciales && (
              <>
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={buildChartDataVotosEspeciales(circuite.votosEspeciales)}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} />
                      <XAxis dataKey="categoria" tick={{ dy: 12 }} />
                      <YAxis tickLine={false} axisLine={true} />
                      <Tooltip content={<CustomTooltip />} />
                      <Legend content={<CustomLegend label="Votos especiales:" />} />
                      {getTiposEspecialesUnicos(circuite.votosEspeciales).map((tipo) => (
                        <Bar
                          key={tipo}
                          dataKey={tipo}
                          stackId={stacked ? "a" : undefined}
